...
-정적변수는 JVM(자바가상머신)에 의해 프로그램이 시자고디는 시점에 초기화된다.
...
class MyStatic {
	/* 멤버변수 : 인스턴스형 멤버변수는 반드시 클래스의 인스턴스를 생성한
	 		후 참조변수를 통해 접근해야한다. 즉, 일반적인 멤버변수는
	 		인스턴스를 생성하는 시점에 메모리에 로드된다.
	 		(멤버변수는 stack영역에, 인스턴스는 Heap영역에 생성됨)
	 */
	int instanceVar;
	
	/* 정적멤버변수 : 인스턴스를 생성할 필요없이 클래스명으로 바로
	 			접근할 수 있다. JVM(자바가상머신)이 프로그램을 시작할때
	 			이미 메모리(Method영역)에 로드된다.*/
	static int staticVar;
	
	/* 인스턴스형 멤버메서드 : 인스턴스형 메서드에서는 모든 멤버를 사용 할
	...
	/* 정적 멤버메서드 : 정적 메서드 내부에서는 인스턴스형 멤버에는 
	 		접근할 수 없다. 정적 멤버만 접근할 수 있다. 
	 		(※ main 메서드에서는 static으로 선언된 메서드만 호출할
	 		수 있었음을 기억하자.)*/
...
public class E01StaticModifier {
	...
	public static void main(String[] args) {
		...
		/* 2개의 인스턴스를 생성한다. 메모리(Heap영역)에는 ms1, ms2
		가 각각 생성된다. */
...
//		MyStatic.instanceMethod();//에러발생
		...
		/*
		결론 : 일반적인 인스턴스형 멤버는 인스턴스 참조변수를 통해
