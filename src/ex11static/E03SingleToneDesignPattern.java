...
할당한 후 이를 프로그램 전체에서 공유해서 사용하고자 할때 쓰는 패턴이다.  

작성방법 
1.생성자의 접근지정자를 private 으로 선언한다.
2.그러면 클래스 외부에서는 생성자에 접근할 수 없으므로 new를 통해
인스턴스 생성을 할 수 없다.
3.클래스 내부에 멤버변수를 static으로 선언하여 인스턴스를 미리 생성해
놓는다.
...
class NoSingleTone {
	...
	클래스 외부 혹은 다른 패키지에서는 접근할 수 없기 때문이다. */
	public NoSingleTone() {}
}
//싱글턴 패턴이 적용된 클래스 정의
class SingleTone {
	//일반적인 멤버변수 선언
	int shareVar;
	/* 정의한 클래스와 동일한 타입으로 선언도니 정적 멤버변수로 JVM(자바
	가상머신)에 의해 프로그램이 시작 될 때 미리 Method영역에 로드되어
	사용할 준비를 마치게된다. */
	private static SingleTone single = new SingleTone();
	
	/* 생성자를 private으로 선언하면 클래스 외부에서는 호출할 수 
	없으므로 new를 통해 인스턴스 생성할 수 없게 된다. */
	private SingleTone() {}
	
	/* 정적메서드로 선언된 getInstance()를 통해 해당 인스턴스의
	참ㄴ조값을 외부로 반환한다. 이런 메서드를 "유틸리티 메서드"라고 한다. */
...
public class E03SingleToneDesignPattern {
	...
	public static void main(String[] args) {
		...
		/* 일반적인 방식의 인스턴스 생성이므로, new로 생성할 때 마다
		새로운 참조값(주소값)을 할당 받게 된다. */
		...
		/* 생성자가 private으로 선언되어 새로운 인스턴스를 생성할 수
		없다. 생서앚가 "not visible" 즉 보이지 않는다는 에러가
		발생한다. */
//		SingleTone st1 = new SingleTone();//에러발생
		
		/* 정적메서드이므로 클래스명을 통해 직접 호출할 수 있다. 해당
		...
		/* 결국 동일한 인스턴스를 사용한 것이므로 마지막에 할당된
		값이 출력된다. */
