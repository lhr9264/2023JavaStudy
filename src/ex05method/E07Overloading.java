...
: 동일한 이름의 메서드를 2개이상 정읭하는것을 말한다.
-매개변수의 갯수가 다르거나 자료형이 다를때 성립한다.
-반환타입만 다른것은 허용하지 않는다.
-컴파일러는 메서드 호출시 전다로디는 인수를 통해 호출할 메서드를 구분한다.
 */
public class E07Overloading {
	...
	person(10)으로 호출한다 가정했을때 어떤 메서드를 호출할지 애매하므로
	에러가 발생된다. 따라서 아래의 메서드는 오버로딩이 성립하지 않는다.*/
...
//	}
	public static void main(String[] args) {
		...
		메서드이다. 정수, 실수, 문자 등 모든 타입에 대해 출력 할 수 있도록
		정의되어 있다.*/
