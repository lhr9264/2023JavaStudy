...
/*
 if문 형식1
 	...
 	-즉 결과값이 boolean타입으로 true/fasle를 반환해야한다.
...
//		}
		...
		/* if문의 조건은 아래와 같이 boolean값을반환하는 조건식이어야 한다. */
		if(num%2==0) {
		...
		//경우에 따라 무조건 실행되는 조건문이 필요한경우 사용한다.
		if(true) {
		...
		if(조건식); => 세미콜론은 문장의 끝을 나타내므로 조건에 상관없이
			중괄호에 있는 문장이 실행된다. 따라서 사용에 주의해야한다.
		*/
		if(num%2 != 0);
		{
		...
		/*
		조건 검사후 실행할 문장이 1개인 경우에는 중괄호를 생략할수 있다.
		단, 실행문이 2개 이상이라면 한 문장만 if문에 포함되므로 반드시
		중괄호를 기술해야한다. 
		*/
		if(num%2 == 0)
			System.out.println("\n"+ num +"은 짝수이다.");
		...
		*/
		...
		/* 아스키코드를 알고있을때의 구현방법. 문자 0과 0가 아스키코드
		...
		메모리에 저장시 아스키코드로 변화되므로 아래와 같은 조건으로도
		확인 할 수 있다. */
