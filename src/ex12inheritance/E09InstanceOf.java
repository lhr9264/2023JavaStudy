...
public class E09InstanceOf {
	...
	모두 받을 수 있다. 
	-Box타입으로 인수를 받게되면 모두 자동형변환(업캐스팅) 되므로 자식
	클래스의 멤버메서드는 호출할 수 없게 된다.
	-이때 자식의 멤버메서드는 호출하기 위해 강제형변환(다운캐스팅)이
	필요하다.
	...
	static void wrapBox(Box b) {
		/*
		세가지 인스턴스 중 어떤것이 전달되더라도 Box타입으로 자동형변환
		되므로 해당 매개변수를 통해 호출할 수 있는 메서드는boxWrap()
		이 유일하다. 
		따라서 다운캐스팅이 필요하다.
		 */
	...
	/*
	 * 123123
	만약 부모타입의 매개변수로 자식 인스턴스를 받을 수 없다면 아래와
	같이 각각의 타입에 따른 모든 메서드를 정의해야 한다.
	...
	복잡해지게 될것이다. 따라서 부모로 자식을 참조할 수 있는것은 개발자에게
	편리함을 가져다준다. 
	 */
