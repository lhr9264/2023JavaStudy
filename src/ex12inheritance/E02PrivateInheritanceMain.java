...
class Account {
	...
	/* 이와같이 개발자가 생성자를 직접 정의하면 디폴트 생성자는 추가되지
	않는다. 디폴트 생성자는 개발자가 생성자를 정의하지 않았을때만
	컴파일러에 의해 자동으로 추가된다. */
	...
	/* 현재는 부모와 자식 클래스가 동일한 패키지에 정의되어 있으므로 접근
	지정자가 default이더라도 접근은 허용된다. 즉 protected를 지워도
	상관없다. */
...
class SavingAccount extends Account {
	...
	현재 부모클래스에는 매개변수가 하나인 생서앚가 유일하므로 만약 아래와
	같이 호출하지 않으면 에러가 발생한다. */
	public SavingAccount(int initVal) {
		/* 만약 아래 문장을 삭제하면 즉시 에러가 발생한다. 삭제하는 겨웅
		super()라는 문장, 즉 매개변수가 없는 디폴트 생성자를 호출하는
		문장이 자동으로 추가되는데, 부모클래스에는 이런 형태의 생성자가
		없으므로 아래와 같이 명시해야한다.*/
		super(initVal);
	}
	/* 부모클래스에 정의도니 멤버변수 money는 private이므로 자식쪽에서는
	아래와 같이 직접 접근할 수 없다. 보이지 않으므로 에러가 발생한다. 
	따라서 부모클래스에 protected로 선언된 메서드를 통해 입금처리를
	해야한다. */
...
public class E02PrivateInheritanceMain {
	...
	public static void main(String[] args) {
		...
		접근할 수 없다. 은닉된 정보이므로 Not visible에러가 발생한다. */
//		sa.money = 100000;//에러발생
		...
		변수를 통해서도 멤버변수 money에 접근할 수 없다. 현재 접근하려는
		위치가 클래스 외부이기 때문이다. 즉 private멤버는 무조건 클래스
		내부에서만 접근을 허용한다. */
