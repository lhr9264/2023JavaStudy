...
: X has a Y => X가 Y를 소유(포함)하다. 여기서는 권총을 소윻나
경찰을 표현한다.

 Has-A와 같이 소유의 관계를 상속으로 표현하면 클래스간의 강한 연결고리가
 형성되어 권총이 없는 경찰을 표현하기가힘들어진다. 따라서 이 경우에는
 구성(Composition)관계로 표현하는것이 적합하다.
 구성관계란 클래스의 멤버로 또 다른 클래스를 사용하는것을 말한다. 
 */
//권총을 추상화한 클래스
class Gun {
	int bullet;
...
class Police {
	...
	public Police(int bNum, int hCuff) {
		...
		인스턴스가 없는것을 의미한다. 즉 Heap 영역에 생성돼 인스턴스가 아예
