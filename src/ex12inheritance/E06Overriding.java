...
오버로딩과 다른점은 매개변수의 갯수 및 타입, 반환타입 등이 완전히 똑같은
형태로 정의하는것이다.
...
class Speaker {
	...
	/* 멤버변수가 private이므로 외부에서 값을 설정할 수 있도록
	setter를 정의. */
...
class BaseSpeaker extends Speaker {
	//베이스 기능의 확장을 위한 멤버변수와 이를 설정하기위한 setter정의
	...
	@Override : 어노테이션이라 부르고, 오버라이딩된 메서드에 추가하여
	 	컴파일러에게 알려주는 역할을 한다. 필수사항은 아니므로 생략할 수
	 	있다.
	 */
	@Override
	public void showState() {
		/* super를 통해 부모클래스에 정의도니 멤버메서드를 호출한다.
...
public class E06Overriding {
	public static void main(String[] args) {
		...
		호출된다. 부모쪽의 메서드는 자식에 의해 가려지게 된다. 
		윈도우 탐색기에서 같은 이름의 파일을 붙여넣기 할때 덮어쓸지
		물어보는것과 동일하다 생각하면된다. */
		...
		/* 단, 부모타입의 변수로 자식 인스턴스를 참조함녀 접근할 수 있는
		범위가 부모인스턴스 까지로 제한되므로, 자식쪽에 정의된 setBase()
		는 호출할 수 없다. */
