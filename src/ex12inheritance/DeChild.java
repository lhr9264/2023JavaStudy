...
public class DeChild extends DeParent{
	...
	public DeChild(String name, int age, String stNumber) {
		/* 자식은 부모의 생성자 호출을 통해 먼저 부모인스턴스를
		 생성한 후 자신의 인스턴스를 생성해야 한다. */
		super(name, age);
		this.stNumber = stNumber;
	}
	/* 자식클래스에서 확장한 메서드로 부모클래스에는 정의되어 
	있지않다. */
	public void study() {
...
//		System.out.println(name +"이(가) 공부한다.");
	...
	/*
	오버라이딩 아님 : 부모클래스에서 private 멤버로 선언하
	 	였으므로 자식쪽에서는 보이지 않아 오버라이딩의 대상이
	 	될수없다. 해당 메서드는 자식쪽에서 새롭게 확장한
	 	메서드가 된다.
	 */
	private void eat() {
	...
	/*
	메서드 오버라이딩의 조건
	-부뫀ㄹ래스에서 선언될때와 비교해서 접근범위가 동일하거나
	그보다 넓으면 가능하다.
	...
	/*
	static(정적) 메서드는 오버라이딩의 대상이 될수 없다. 클래스
	 	외부(메서드영역)에 미리 로드되므로 오버라이딩 되지않고,
	 	호출시 각 클래스명을 통해 사용해야한다. 	
	 */
