...
public class E07RideAndLoad02 {
	...
	public static void main(String[] args) {
		...
		따르고, 오버라이딩의 대상이 될 수 없다. 인스턴스 내부가 아닌
		외부(Method영역)에 이미 로드되어 있는 상태이기 때문이다. */
		...
		/*
		오버라이딩 처리도니 메서드이므로 참조변수의 상관없이 항상
		자식쪽의 메서드가 호출된다.
		자식인스턴스가 생성될때 부모쪽의 메서드가 가려지게 되므로
		참조변수의 영향을 전혀 받지 않는다. 
		 */
		parent.excecise();
		parent.sleep();
		...
		//정적메서드는 오버라이딩 되지않는다. 클래스명으로 호출하면 된다.
