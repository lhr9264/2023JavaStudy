...
/* 
static(정적) 타입의 멤버도 상속이 되고, 하위클래스에서 접근시 멤버변수의
이름만으로 접근이 가능하다.
단, 클래스 외부에서 접근할 경우 인스턴스 생성없이 클래스명을 통해서
접글할 수 있다.
즉, static이 가진 기본규칙을 따른다.
 */
class Adder {
	/* static으로 선언된 멤버변수는 실제로는 클래스외부(Method영역)에
	생서되어 있으므로 클래스명을 통한 접근이 허용된다.
...
/* 부모의 멤버변수가 public 이므로 접근의 제한이 없다. 따라서 자식
클래스에 작성도니 모든 코드는 문제없이 실행된다. */
...
public class E03StaticInheritance {
	...
	public static void main(String[] args) {
		...
		/* 앞에서 호출한 메서드는 모두 메서드영역에 생성된 정적변수
