...
'복사'를 통해 값이 전달된다. 
그래서 다른 지여그이 메서드에서 값이 변경이 있떠라도 호출한 메서드의 값엥는
전혀 영향을 미치지 않는다.
메모리적 관점에서 보면 스택영역만 사용하는 케이스로 main과 호출된 메서드는
서로 다른 지역이므로 서로에게 전혀 영향을 미치지 않는것이 특징이다.
 */
public class E03CallByValue {
	...
	public static void main(String[] args) {
		...
		/* 매객변수로 전달된 값은 복사되어 다른 지여그이 메서드 내에서만
		밴경되므로 main메서드에서는 초기값이 그대로 유지된다. */
	...
	public static void callByValue(int fNum, int sNum) {
		...
		sNum = temp;
		/*
		해당 메서드 내에서는 값이 교환이 완료되었기 때문에 출력결과는
		200, 100이 된다. 
		 */
		System.out.println("[callByValue메소드안]"
				+ "first="+fNum+",second="+sNum);
		/* 해당 메서드의 실행이 종료되면 메모리에서는 소멸되고, 호출한
		지점으로 되돌아간다. */
	}
