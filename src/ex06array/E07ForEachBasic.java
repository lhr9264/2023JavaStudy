...
/* 
foreach문(개선된 for문)
: 배열의 일부가 아니라 전체를 참조해야할대 주로 사용하는 for문을 말한다.
배열의 원소를 참조의 형태로 가져와서 사용한다.
따라서 참조의 목적으롬나 사용해야하고, 값의 변경은 허용되지 않는다.
형식]
 	for(자료형 참조변수 : 배열명){
 		실행문장;
 		참조변수를 통해 원소 출력가능;
...
public class E07ForEachBasic {
	...
	public static void main(String[] args) {
		...
		아래 증가연산자를 토앻 증가시킨값은 실제 배열에 적용되지 않는다.
		해당 for문 내에서만 증가된 값이 출력된다. */
		for(int b : arr) {
			//배열으 ㅣ각 원소를 꺼내서 b에 잠깐 저장한다.
			b++;
			//1증가 시킨 후 출력한다.
			System.out.print(b + " ");
			//따라서 원 배열에는 적용되지 않는다.
		...
		/* 1증가되는 부분이 기존 배열에 적용되지 않으므로 초기화헀던
		상태 그대로를 유지한다. */
		for(int c : arr) {
			System.out.print(c + " ");
		}
