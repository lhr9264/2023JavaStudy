...
-반환값이 없다. 따라서 void를 사용하면 에러가 발생한다.
...
public class PersonConstructor {
	...
	public PersonConstructor(String name) {
		/* 해당 this는 멤버변수와 매개변수를 구분하기 위한 용도로
		사용된다. 클래스 자신, 즉 멤버를 가리킨다. 따라서 좌측항은
		멤버변수, 우측항은 매개변수가 된다. */
		this.name = name;
		age = 1;
		addr = "출처불명";
	...
	//멤버메서드 : 멤버변수를 초기화를 목적으로 정의한다.
	void initialize(String name, int age, String addr) {
		/* 해당 코드는 생성자 내에서만 사용할 수 있다. 생성자는 인스턴스
		생성시 딱 한번만 호출되고, 개발자가 임의로 호출할 수 없다.
		하지만 멤버메서드는 개발자가 원할때 언제든 호출이 가능하므로
		생성자와는 용도와 특성이 다르다.
