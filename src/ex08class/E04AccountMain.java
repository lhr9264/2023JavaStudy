...
//	String accountNumber;
	...
	선언하면 된다. 이 경우 가독성을 위해 숫자 사이에 _(언더바)를 사용하는것이
	허용된다. 단, 정수타입으로 작성시 "은 제거해야한다. */
	long accountNumber;
	/* 게좌번호나 전화번호같은 데이터를 표현할때는 위와 같이 String 타입이나 
	long 타입을 통해 표현할 수 있다. */
	...
	void withdraw(int money) {
		...
		if(balance >= money) {
			...
			System.out.println("잔고부족응로 출금불능"); 
		}
	}
	...
	//초기화메서드 : 멤버변수를 매개변수로 전달된값을 통해 초기화한다.
...
public class E04AccountMain {
	public static void main(String[] args) {
		...
		/* 멤버변수를 직접 기술하는것은 초기화 메서드에 비해 불편한 방법
		이므로 위와같이 초기화하는것이 좋다. */
