...
class FruitSeller {
	...
	/* 멤버변수의 경우 초긱밧이 없는 상태로 정의한 후 인스턴스를
	생성하고 차후 초기화 할 수 있다.
	하지만 멤버상수는 인스턴스 생성과 상관없이 정의하는 시점에 반드시
	초기화해야한다. 즉 1000을 지우면 에러가 발생한다. */
	
	//멤버메서드
	/*
	판매자가 사과를 판매하는 행위를 표현한다.
	1. 구매자로부터 금액을 받는다.(메서드 호출시 매개변수를 통해 받는다)
	2. 금액/단가=판매도리 사과의 갯수를 계산한다.(5000/1000=5개)
...
class FruitBuyer{
	...
	1. 판매자에게 금액을 지불한다. => 여기에서 seller 인스턴스의
		saleApple()메서드가 호출된다.
	2. 판매자에게 금액에 해당하는 사과의 갯수를 반환하면, 해당 갯수만큼
	보유한 사과가 증가한다.
		=> += 메서드호출(5000);
...
public class E06FruitSalesMain1 {
	...
	public static void main(String[] args) {
		...
		/* 판매자와 구매자의 인스턴스를 생성한다. 여기서는 우리가 직접
		생성자를 만들지 않았으므로 자동으로 생성되는 디폴트 생성자를 통해
		인스턴스를 생성하게 된다. */
