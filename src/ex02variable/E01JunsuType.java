...
public class E01JunsuType {
	...
	public static void main(String[] args) {
		...
		-즉 정수형 변수를 사용할때에는 거의 대부분 int형을 사용하면 된다.
		 */
		//int형 변수를 선언한 후 초긱밧을 할당한다.
		int int1 = 100;
		int int2 = 200;
		...
		-CPU는 정수의 연상닌경우 int형에 최적화 되어있기 때문이다.
		-byte, short형은 게임캐릭터의 움직임이나 음원파일 등을 표현할때 주로 사용된다.
		 */
		...
		/*
		-동일 자료형끼리의 연산은 동일자료형이 되는것이 기본 우너칙이다.
		-하지만 큰 자료형과 작은 자료형을 연산하면 큰 자료형으로 자동형변환된다.
		 이유는 데이터의 손실(오차)이 있을수 있기 때문이다.
		 */
		...
		이와 같이 할당을 위한 입력되는 숫자도 자료형을 기반으로 메모리에 로드되는데 이를 가르켜 리터럴(literal)이라고 한다.
		 */
